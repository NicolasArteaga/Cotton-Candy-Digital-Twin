...
class CottonCandyPipeline:
    ...
    def parse_index_file(self, index_path: Path) -> List[Dict]:
        """
        Parse index.txt file to extract 'Cottonbot - Run with Data Collection vAutonomous' processes
        ...
        try:
            ...
            pattern = r'Cottonbot - Run with Data Collection vAutonomous \(([a-f0-9-]+)\) - (\d+)'
    ...
    def process_batch(self, batch_folder: Path, target_process: Optional[int] = None, 
        ...
        if not processes:
            print(f"  Warning: No 'Cottonbot - Run with Data Collection vAutonomous' processes found in {batch_name}")
            return []
